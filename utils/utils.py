...
def log_step(title: str, payload=None, symbol: str = "🟢"):
    # Avoid Unicode encoding issues on Windows consoles by stripping non-ASCII symbols.
    safe_symbol = symbol.encode("ascii", "ignore").decode() if symbol else ""
    print(f"\n[b]{safe_symbol} {title}[/b]")
    if payload:
